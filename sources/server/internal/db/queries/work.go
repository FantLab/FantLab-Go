...
const (
	...
	WorkChildren = `
		WITH RECURSIVE CTE (work_id, parent_work_id, is_bonus, depth) AS (
			SELECT work_id, parent_work_id, is_bonus, 0 AS depth
			FROM work_links
			WHERE parent_work_id = ?
			UNION ALL
			SELECT wl.work_id, wl.parent_work_id, wl.is_bonus, cte.depth + 1 AS depth
			FROM work_links wl
