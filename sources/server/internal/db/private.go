	...
	err := db.engine.InTransaction(func(rw sqlr.ReaderWriter) error {
		return codeflow.Try(
			func() error { // Создаем новое сообщение
				result := rw.Write(ctx, sqlr.NewQuery(queries.PrivateInsertNewMessage).
			...
			func() error { // Сохраняем текст сообщения
				return rw.Write(ctx, sqlr.NewQuery(queries.PrivateInsertMessageText).WithArgs(messageId, text)).Error
			},
			func() error { // Получаем сообщение
				return rw.Read(ctx, sqlr.NewQuery(queries.PrivateGetMessage).WithArgs(messageId)).Scan(&message)
			},
			...
			func() error { // Удаляем, если есть, черновик сообщения
				return rw.Write(ctx, sqlr.NewQuery(queries.PrivateCancelMessagePreview).WithArgs(fromUserId, toUserId)).Error
			},
		)
	})
