	...
	g.Subgroup("Для зарегистрированных пользователей", func(g *routing.Group) {
		...
		g.Subgroup("Для пользователей с валидной сессией", func(g *routing.Group) {
			...
			g.Subgroup("С проверкой на бан", func(g *routing.Group) {
				...
				g.Endpoint(http.MethodPost, "/forum_messages/:id/file", api.UploadForumMessageFile, "Загрузка аттача к сообщению в форуме")
				...
				g.Endpoint(http.MethodPost, "/topics/:id/message_draft/file", api.UploadForumMessageDraftFile, "Загрузка аттача к черновику сообщения в форуме")
