	...
	g.Subgroup("Для зарегистрированных пользователей", func(g *routing.Group) {
		...
		g.Subgroup("Для пользователей с валидной сессией", func(g *routing.Group) {
			...
			g.Subgroup("С проверкой на бан", func(g *routing.Group) {
				...
				g.Endpoint(http.MethodPost, "/bookcases/default", api.CreateDefaultBookcases, "Создание первичных книжных полок")
				g.Endpoint(http.MethodPost, "/edition_bookcases", api.AddEditionBookcase, "Создание полки изданий")
				g.Endpoint(http.MethodPost, "/work_bookcases", api.AddWorkBookcase, "Создание полки произведений")
				g.Endpoint(http.MethodPost, "/film_bookcases", api.AddFilmBookcase, "Создание полки фильмов")
