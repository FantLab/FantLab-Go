	...
	g.Subgroup("Для зарегистрированных пользователей", func(g *routing.Group) {
		...
		g.Subgroup("Для пользователей с валидной сессией", func(g *routing.Group) {
			...
			g.Subgroup("С проверкой на бан", func(g *routing.Group) {
				...
				g.Endpoint(http.MethodPost, "/bookcases", api.AddBookcase, "Создание книжной полки")
				g.Endpoint(http.MethodPut, "/bookcases/:id", api.EditBookcase, "Редактирование книжной полки")
				...
				g.Endpoint(http.MethodPost, "/bookcases/:id/items", api.AddBookcaseItem, "Добавление item-а на книжную полку")
