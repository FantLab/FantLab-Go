...
func (db *DB) FetchForums(availableForums []uint16) ([]Forum, error) {
	...
	err := db.R.Read(fetchForumsQuery.WithArgs(availableForums).Rebind()).Scan(&forums)
...
func (db *DB) FetchModerators() (map[uint32][]ForumModerator, error) {
	...
	err := db.R.Read(fetchModeratorsQuery).Scan(&moderators)
...
func (db *DB) FetchForumTopics(availableForums []uint16, forumID uint16, limit, offset uint32) (*ForumTopicsDBResponse, error) {
	...
	err := db.R.Read(forumExistsQuery.WithArgs(forumID, availableForums).Rebind()).Scan(&forumExists)
	...
	err = db.R.Read(fetchTopicsQuery.WithArgs(forumID, limit, offset)).Scan(&topics)
	...
	err = db.R.Read(topicsCountQuery.WithArgs(forumID)).Scan(&count)
...
func (db *DB) FetchTopicMessages(availableForums []uint16, topicID, limit, offset uint32, sortDirection string) (*ForumTopicMessagesDBResponse, error) {
	...
	err := db.R.Read(shortTopicQuery.WithArgs(topicID, availableForums).Rebind()).Scan(&shortTopic)
	...
	err = db.R.Read(topicMessagesCountQuery.WithArgs(topicID)).Scan(&count)
	...
	err = db.R.Read(fetchTopicMessagesQuery.Format(sortDirection).WithArgs(topicID, finalOffset+1, finalOffset+int32(limit))).Scan(&messages)
